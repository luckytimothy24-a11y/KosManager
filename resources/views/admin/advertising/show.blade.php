    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.advertising.index') }}" class="w-9 h-9 inline-flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
            ...
            <div class="flex items-center gap-2">
                @if($campaign->kos_id === null && ! in_array($campaign->status, [
                    \App\Models\AdvertisingCampaign::STATUS_COMPLETED,
                    \App\Models\AdvertisingCampaign::STATUS_CANCELLED,
                ]))
                    <a href="{{ route('admin.advertising.edit', $campaign) }}" class="inline-flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-4 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                ...
                @if($campaign->status === \App\Models\AdvertisingCampaign::STATUS_PENDING_REVIEW)
                    <form method="POST" action="{{ route('admin.advertising.approve', $campaign) }}">
                        @csrf
        ...
        <div id="reject-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <form method="POST" action="{{ route('admin.advertising.reject', $campaign) }}" class="relative bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-xl">
                @csrf
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Tolak Kampanye</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Berikan alasan penolakan yang akan dilihat owner.</p>
        ...
        <div id="suspend-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <form method="POST" action="{{ route('admin.advertising.suspend', $campaign) }}" class="relative bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-xl">
                @csrf
