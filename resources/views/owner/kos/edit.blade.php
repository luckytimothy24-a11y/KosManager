<x-app-layout>
    ...
    <div class="max-w-2xl">
        <x-alert />
        <form method="POST" action="{{ route('owner.kos.update', $kos) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf @method('PUT')
            ...
            <section class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 sm:p-8"
                     x-data="{ hasExisting: @bool($kos->photo && @file_exists(public_path('storage/'.$kos->photo))), previewUrl: null }">
            ...
            <section class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 sm:p-8 space-y-5">
                ...
                <div>
                    ...
                    <textarea id="general_facilities" name="general_facilities" rows="2"
                              class="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-100 text-sm focus:border-primary-500 focus:ring-primary-500">{{ old('general_facilities', $kos->general_facilities) }}</textarea>
                </div>
