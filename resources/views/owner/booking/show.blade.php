                        ...
                        <div class="flex flex-wrap items-center justify-end gap-3">
                            @if($booking->status === 'pending')
                                <form method="POST" action="{{ route("$prefix.booking.reject", $booking) }}"
                                      onsubmit="return confirm('Tolak booking ini? Penyewa akan menerima notifikasi penolakan.')">
                                    @csrf
                                    <button type="submit"
                                            class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-xl hover:bg-red-100 dark:hover:bg-red-500/20 transition">
                                        <i class="ri-close-circle-line"></i> Tolak Booking
                                    </button>
                                </form>
                                <form method="POST" action="{{ route("$prefix.booking.approve", $booking) }}"
                                      onsubmit="return confirm('Setujui booking ini? Kamar akan direservasi untuk penyewa.')">
                                    @csrf
                                    <button type="submit"
                                            class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 active:bg-green-800 rounded-xl transition shadow-sm shadow-green-600/30">
